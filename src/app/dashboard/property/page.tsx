...
import { PROPERTY_AGENT_PLANS } from "@/lib/constants";
...
export default function PropertyDashboardPage() {
  ...
  async function fetchData() {
    try {
      ...
      const listingsRes = await fetch(`/api/listings?userId=${currentUser.id}&category=property`);
      if (listingsRes.ok) {
        const listingsData = await listingsRes.json();
        const items: PropertyListing[] = listingsData.listings || listingsData || [];
        setListings(items);
  ...
  function getPropertyType(subcategory: { name: string } | null): string {
    if (!subcategory) return "Property";
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Property Agent Dashboard</h1>
        <p className="text-sm text-gray-500 mt-1">
          Manage your property listings and grow your real estate business.
        </p>
      </div>
      ...
      {/* Property Agent Plans */}
      <section className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-2">Property Agent Plans</h2>
        <p className="text-gray-600 text-sm mb-6">
          Choose a plan that fits your agency size and start listing properties today.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PROPERTY_AGENT_PLANS.map((plan, index) => (
            <div
              key={plan.id}
      ...
      <section className="bg-white rounded-lg shadow p-6">
        ...
        {listings.length > 0 ? (
                  ...
                  <tr key={listing.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-2">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                        {getPropertyType(listing.subcategory)}
                      </span>
                    </td>
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 mb-3">You have no property listings yet.</p>
            <a
              href="/listings/new"
